...
def readSpikeSourceDataFile(path):
    with open(path, 'rb') as csvfile:
        timesReader = csv.reader(csvfile, delimiter=',')
        spikeLists = {'spike_times': [list(map(int, line)) for line in timesReader]}
    return spikeLists
...
def plotSpikes(popn,totalSimulationTime, indexOffset):
    ...
    plt.plot([i[1] for i in spikes], [i[0] for i in spikes],"ko", markersize = 2) #black pixels
